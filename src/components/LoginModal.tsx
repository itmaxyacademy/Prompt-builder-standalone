...
export function LoginModal({ isOpen, onClose, user, onLogin, onLogout }: LoginModalProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://api.maxy.academy/api/v1/auth/login", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Accept": "application/json"
        },
        body: JSON.stringify({ email, password, client_app: "web" })
      });
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl relative">
        <div className="flex items-center justify-between p-4 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900">
            {user ? "Your Profile" : "Login to Prompt Builder"}
          </h2>
          <button
            onClick={onClose}
          ...
          {user ? (
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold transition-all shadow-md hover:opacity-90 mt-2 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                <LogIn size={16} />
